...
*   **Визуализация:** Grafana (`http://<server_ip>:3000`) строит графики на основе собранных метрик.
*   **Оповещения в Telegram:** В целях безопасности токен Telegram-бота и ID чата исключены из конфигурационных файлов репозитория. Оповещения настраиваются вручную через веб-интерфейс Grafana (раздел **Alerting** ➔ **Contact points** ➔ тип **Telegram**) и надежно сохраняются в постоянном томе `grafana-data`.

### Централизованное логирование
*   Filebeat автоматически отслеживает лог-файлы Docker-контейнеров на хосте и стримит их в базу данных Elasticsearch.
*   Kibana (`http://<server_ip>:5601`) используется для удобного поиска, фильтрации логов и отслеживания ошибок приложений в реальном времени.
